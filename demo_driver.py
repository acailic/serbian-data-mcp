...
async def cmd_dataset(did):
    ...
    if ds.organization:
        ...
        if ds.organization.acronym:
            print(f"  {bold('Acronym:')}      {ds.organization.acronym}")
    print()
    if ds.resources:
...
async def cmd_download(rid):
    ...
    c = await _cli()
    try:
        data = await c.get_resource_data(rid)
    except Exception as e:
        print(f"  {yellow('⚠')}  {e}")
        return
    if data is None:
    ...
    if hasattr(data, "shape"):
        ...
        if data and isinstance(data[0], dict):
            ...
            for row in data[:3]:
                print(f"    {dim(str(row)[:90])}")
    elif isinstance(data, dict):
        _last_data = []
        print(f"  {green('✓')}  Dict with {len(data)} keys: {', '.join(list(data.keys())[:10])}")
    else:
        _last_data = []
        print(f"  {green('✓')}  {type(data).__name__}: {str(data)[:200]}")
    print()
...
async def cmd_filter():
    ...
    if filters:
        ...
        print(f"  {yellow('⚠')}  Could not auto-detect")
    print()
...
async def cmd_visualize():
    ...
    from serbian_data_mcp.viz.exporters import export_html, fig_to_dict

    df = pd.DataFrame(_last_data)
    nc = df.select_dtypes(include=["number", "int64", "float64"]).columns.tolist()
    sc = df.select_dtypes(include=["object", "string"]).columns.tolist()
    if len(sc) >= 2 and len(nc) >= 1:
        x, y, t = sc[0], nc[0], "bar"
    elif len(nc) >= 2:
        x, y, t = nc[0], nc[1], "scatter"
    elif len(nc) >= 1:
        x, y, t = nc[0], None, "histogram"
    else:
        print(f"  {yellow('⚠')}  Not enough data")
        return
    print(f"  {bold('Chart:')} {t}  x={x}  y={y or 'auto'}\n")
    try:
        b = ChartBuilder(_last_data)
        if t == "bar":
            fig = b.bar_chart(x, y, title="Serbian Data Demo")
        elif t == "scatter":
            fig = b.scatter_plot(x, y, title="Serbian Data Demo")
        else:
            fig = b.histogram(x, title="Serbian Data Demo")
        fd = fig_to_dict(fig)
...
async def cmd_orgs():
    ...
    c = await _cli()
    try:
        orgs = await c.list_organizations(page_size=8)
    except Exception as e:
        print(f"  {yellow('⚠')}  {e}")
        return
    ...
    for i, o in enumerate(orgs, 1):
        ...
        if o.acronym:
            print(f"     {dim('Acronym:')} {o.acronym}")
        if badges:
            print(f"     {dim('Badges:')} {badges}")
        if o.business_number_id:
            print(f"     {dim('PIB:')} {o.business_number_id}")
...
async def cmd_stats():
    ...
    c = await _cli()
    try:
        d = await c._request("GET", "/api/1/datasets/", params={"rows": 1})
        tds = d.get("total", 0)
        o = await c._request("GET", "/api/1/organizations/", params={"rows": 1})
        to = o.get("total", 0)
    except Exception:
        tds, to = 3430, 182
    print()
...
async def main():
    ...
    h = {
        ...
        "dataset": lambda: cmd_dataset(arg),
        "download": lambda: cmd_download(arg),
        "profile": cmd_profile,
        "filter": cmd_filter,
